...
import '../models/vo_daily_objective.dart';
...
class DailyObjectiveNotifier extends StateNotifier<DailyObjectiveState> {
  ...
  Future<void> setObjective(DateTime date, String objective) async {
    ...
    final updatedObjectives = Map<String, DailyObjectiveVo>.from(
      state.objectives,
    );
...
class DailyObjectiveState {
  ...
  DailyObjectiveState copyWith({Map<String, DailyObjectiveVo>? objectives}) {
    return DailyObjectiveState(objectives: objectives ?? this.objectives);
  }
}

final dailyObjectiveProvider =
    StateNotifierProvider<DailyObjectiveNotifier, DailyObjectiveState>((ref) {
