...
import '../models/vo_category.dart';
...
class CategoryNotifier extends StateNotifier<CategoryState> {
  ...
  Future<void> _loadCategories() async {
    try {
      ...
      if (categoriesJson != null) {
        ...
        final categories =
            categoriesList.map((json) => CategoryVo.fromJson(json)).toList();
  ...
  Future<void> updateCategory(
    ...
    final updatedCategories =
        state.categories
            .map(
              (category) =>
                  category.name == oldName ? updatedCategory : category,
            )
            .toList();
    state = state.copyWith(categories: updatedCategories);
    await _saveCategories();
  ...
  Future<bool> archiveCategory(String categoryName) async {
    ...
    final activeCategories =
        state.categories.where((c) => !c.isArchived).toList();
