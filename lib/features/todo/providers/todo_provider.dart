...
import '../models/todo_items_dummy.dart';

class TodoNotifier extends StateNotifier<TodoState> {
  TodoNotifier() : super(TodoState(selectedTodo: null, allTodos: [])) {
    _loadTodos();
  }
  ...
  Future<void> _loadTodos() async {
    try {
      ...
      state = state.copyWith(allTodos: todos, selectedTodo: selectedTodo);
    } catch (e) {
      // 로드 실패시 더미 데이터로 초기화
  ...
  Future<void> _saveTodos() async {
    try {
      ...
      final todosJson = json.encode(
        state.allTodos.map((todo) => todo.toJson()).toList(),
      );
  ...
  Future<void> deleteTodo(String todoId) async {
    final updatedTodos =
        state.allTodos.where((todo) => todo.id != todoId).toList();
  ...
  Future<void> toggleTodoComplete(String todoId) async {
    ...
    TodoItemVo? newSelectedTodo = state.selectedTodo;
    if (state.selectedTodo?.id == todoId) {
      final completedTodo = updatedTodos.firstWhere(
        (todo) => todo.id == todoId,
      );
  ...
  // 할일을 다른 카테고리로 이동
  Future<void> moveTodoToCategory(
    String todoId,
    String newCategory,
    Color newColor,
...
class TodoState {
  ...
  TodoState({required this.selectedTodo, required this.allTodos});

  TodoState copyWith({TodoItemVo? selectedTodo, List<TodoItemVo>? allTodos}) {
    return TodoState(
